...
pokemon.post("/",async (req,res,next) => {
	...
	return res.status(201).json({code:201 , message:"pokemon aparecio milagrosamente de repente"});
	}
...
pokemon.put("/:id([0-9]{1,3})",async(req,res,next)=> {
	...
	return res.status(200).json({code:200 , message:"pokemon se adapto a nuevas circustancias"});
	}
...
pokemon.patch("/:id([0-9]{1,3})",async(req,res,next)=> {
	...
	return res.status(500).jspon({code:500 , message: " Ocurrio un accidente deal with it "});
} 
	return res.status (500).json({code:500, message:"Campos incompletos"});
});
